  ...
  async ({ event, step }) => {
    ...
    await step.run("log-detailed-results", async () => {
      ...
      for (const batch of structuredSummary.batches) {
        console.log(
          `[Batch ${batch.batchNumber}] Records: ${batch.recordIds.join(", ")}`,
        );
        const { zai, mistral, gemini } = batch.modelUsage;
        console.log(
          `  Model usage: Zai(${zai}), Mistral(${mistral}), Gemini(${gemini})`,
        );
...
export const dummyEnv = {
  ...
  R2_ENDPOINT:
    ...
    "https://4c7c10d0a0b9ffcead7f92c375ec9f12.r2.cloudflarestorage.com",
  R2_BUCKET: process.env.R2_BUCKET ?? "bucket-competition",
  R2_PUBLIC_URL:
    process.env.R2_PUBLIC_URL ?? "https://objectcompetition.wahyuikbal.com",
  WAHA_BASE_URL:
    process.env.WAHA_BASE_URL ?? "https://waha-qxjcatc8.sumopod.in",
  WAHA_API_KEY: process.env.WAHA_API_KEY ?? "",
  WA_SESSION_ID:
    process.env.WA_SESSION_ID ?? "session_01jx523c9fdzcaev186szgc67h",
  CHAT_1: process.env.CHAT_1 ?? "120363408171323961@newsletter",
  CHAT_2: process.env.CHAT_2 ?? "120363407021321600@newsletter",
} as const;
 ...
 */
export const functions = [processDraftBatchesFn];
