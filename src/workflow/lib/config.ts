...
export function createConfig(
  ...
  return {
    ...
    r2: {
      ...
      publicUrl: getEnv(
        "R2_PUBLIC_URL",
        "https://objectcompetition.wahyuikbal.com",
      ),
      maxAttempts: 3,
      baseDelayMs: 1000,
      maxDelayMs: 10000,
    ...
    whatsapp: {
      ...
      channelIds: [
        env.CHAT_1 ?? "120363408171323961@newsletter",
        env.CHAT_2 ?? "120363407021321600@newsletter",
      ],
    },
    db: {
      ssl: "require",
...
export function validateConfig(
  ...
  if (hasR2KeyId !== hasR2Secret) {
    missing.push(
      "R2_ACCESS_KEY_ID and R2_SECRET_ACCESS_KEY (both required if one is set)",
    );
  }
