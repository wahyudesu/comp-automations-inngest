...
const R2_BUCKET = "bucket-competition";
const R2_PUBLIC_URL = "https://objectcompetition.wahyuikbal.com";
...
export async function uploadToR2(
  ...
  if (!accessKeyId || !secretAccessKey) {
    ...
    return posts; // Return original posts without uploading
  }
  ...
  for (let i = 0; i < posts.length; i++) {
    ...
    const result = await uploadSingleImage(post, s3Client, config, postLog);
    ...
    if (result.success) {
      ...
      failureCount++;
      postLog.error("Upload failed", undefined, {
        title: post.title?.substring(0, 50),
        error: result.error,
      });
      updatedPosts.push({
